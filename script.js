...
const exportDataModalElement = document.getElementById('exportDataModal');
exportDataModalElement.addEventListener('show.bs.modal', () => {
  document.getElementById('exportedJsonTextarea').value = exportCoreDataAsJson();
...
document.getElementById('copyExportedJsonButton').addEventListener('click', async () => {
  const exportedJsonText = document.getElementById('exportedJsonTextarea').value;
  const copyFeedbackElement = document.getElementById('copyFeedbackText');
  try {
    await navigator.clipboard.writeText(exportedJsonText);
  } catch (error) {
    // 部分環境無法使用 Clipboard API，改用傳統選取方式讓使用者手動複製
    const textareaElement = document.getElementById('exportedJsonTextarea');
...
/* -------------------------------------------------------------------------
   匯入資料
   ------------------------------------------------------------------------- */
const importDataModalElement = document.getElementById('importDataModal');
importDataModalElement.addEventListener('show.bs.modal', () => {
...
document.getElementById('confirmImportButton').addEventListener('click', () => {
  ...
  if (!rawImportText) {
    importErrorAlertElement.textContent = '請先貼上 JSON 內容。';
  ...
  try {
    ...
    importErrorAlertElement.textContent = 'JSON 格式錯誤，請確認貼上的內容完整且格式正確。';
    importErrorAlertElement.classList.remove('d-none');
    return;
  }

  const candidateLevelsById = parsedImportData.coreLevels || parsedImportData;
  const didApplyAnyValue = applyImportedCoreLevels(candidateLevelsById);

  if (!didApplyAnyValue) {
    importErrorAlertElement.textContent = '沒有找到可辨識的核心資料，請確認 JSON 內容是否來自本工具的匯出檔案。';
    importErrorAlertElement.classList.remove('d-none');
    return;
  }
  ...
  const importModalInstance = bootstrap.Modal.getInstance(importDataModalElement);
