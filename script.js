/* =========================================================================
   ...
   /* -------------------------------------------------------------------------
      ...
      ------------------------------------------------------------------------- */
   let coreLevelsById = {};
   let playerOwnedResources = { erda: 0, fragments: 0 };
   let selectedCoreId = null;
   let autoSaveTimerId = null;
   
   const hexPlateElement = document.getElementById('hexaPlateElement');
   const ownedErdaInputElement = document.getElementById('ownedErdaInput');
   const ownedFragmentInputElement = document.getElementById('ownedFragmentInput');
   ...
   function clampNonNegativeInteger(rawValue) {
   ...
   // 根據「玩家目前持有的 Sol Erda / Fragment」，推算把這些資源全部投入後能達到的進度，
   // 以及達到 100% 之前還缺多少 Sol Erda / Fragment
   function calculateProjectedCoverage(totals) {
     const projectedErda = Math.min(totals.totalErdaRequired, totals.totalErdaUsed + playerOwnedResources.erda);
     const projectedFragments = Math.min(totals.totalFragmentsRequired, totals.totalFragmentsUsed + playerOwnedResources.fragments);
     const totalRequired = totals.totalErdaRequired + totals.totalFragmentsRequired;
     const projectedPercent = totalRequired > 0
       ? ((projectedErda + projectedFragments) / totalRequired) * 100
       : 100;
     ...
     return {
       ...
       projectedFragments,
       projectedPercent,
       erdaStillNeeded: Math.max(0, totals.totalErdaRequired - projectedErda),
       fragmentsStillNeeded: Math.max(0, totals.totalFragmentsRequired - projectedFragments)
     };
   }
   ...
   /* -------------------------------------------------------------------------
      ...
      ------------------------------------------------------------------------- */
   function buildProgressRowMarkup(labelText, progressBarColorClass, usedAmountText, percentValue, coveragePercentValue) {
     const clampedPercent = Math.min(100, Math.max(0, percentValue));
     const clampedCoverage = Math.min(100, Math.max(clampedPercent, coveragePercentValue));
     return `
       <div class="progress-row mb-3">
         ...
         <div class="progress" role="progressbar" aria-valuenow="${clampedPercent.toFixed(1)}" aria-valuemin="0" aria-valuemax="100">
           <div class="progress-bar-coverage" style="width:${clampedCoverage}%"></div>
   ...
   // 持有資源尚未輸入時不顯示摘要文字；輸入後顯示「投入後可達到幾%」與「還差多少才能點滿」
   function buildInventoryCoverageSummaryMarkup(coverage) {
     if (playerOwnedResources.erda === 0 && playerOwnedResources.fragments === 0) return '';
     if (coverage.erdaStillNeeded === 0 && coverage.fragmentsStillNeeded === 0) {
       return `<div class="inventory-coverage-note small mt-2">🎉 目前持有資源即可讓這裡全部點滿</div>`;
     }
     return `<div class="inventory-coverage-note small mt-2">投入目前持有資源後可達 <b>${coverage.projectedPercent.toFixed(1)}%</b>，還差 Sol Erda <b>${formatNumberWithLocale(coverage.erdaStillNeeded)}</b> / Fragment <b>${formatNumberWithLocale(coverage.fragmentsStillNeeded)}</b></div>`;
   }
   ...
   function renderProgressSection(containerElementId, totals) {
     ...
     const coverage = calculateProjectedCoverage(totals);
     const erdaCoveragePercent = totals.totalErdaRequired > 0 ? (coverage.projectedErda / totals.totalErdaRequired) * 100 : 100;
     const fragmentCoveragePercent = totals.totalFragmentsRequired > 0 ? (coverage.projectedFragments / totals.totalFragmentsRequired) * 100 : 100;
   
     document.getElementById(containerElementId).innerHTML =
       buildProgressRowMarkup('Sol Erda', 'progress-bar-gold', `${formatNumberWithLocale(totals.totalErdaUsed)} / ${formatNumberWithLocale(totals.totalErdaRequired)}`, erdaPercent, erdaCoveragePercent) +
       buildProgressRowMarkup('Sol Erda Fragment', 'progress-bar-violet', `${formatNumberWithLocale(totals.totalFragmentsUsed)} / ${formatNumberWithLocale(totals.totalFragmentsRequired)}`, fragmentPercent, fragmentCoveragePercent) +
       buildProgressRowMarkup('總進度', 'progress-bar-teal', `${totals.overallPercent.toFixed(1)}%`, totals.overallPercent, coverage.projectedPercent) +
       buildInventoryCoverageSummaryMarkup(coverage);
   }
   ...
   /* -------------------------------------------------------------------------
      玩家持有資源 (Player Inventory)
      輸入框、加減按鈕與清空按鈕的事件都在這裡集中處理，
      任何數值變動都會同步：更新輸入框顯示 → 重新計算進度 → 排程自動儲存
      ------------------------------------------------------------------------- */
   function updateOwnedResourceValue(resourceKey, newValue) {
     playerOwnedResources[resourceKey] = clampNonNegativeInteger(newValue);
     syncOwnedResourceInputsToState();
     refreshAllProgressSections();
     scheduleAutoSave();
   }
   
   function syncOwnedResourceInputsToState() {
     ownedErdaInputElement.value = playerOwnedResources.erda;
     ownedFragmentInputElement.value = playerOwnedResources.fragments;
   }
   
   ownedErdaInputElement.addEventListener('input', (event) => {
     updateOwnedResourceValue('erda', event.target.value);
   });
   ownedFragmentInputElement.addEventListener('input', (event) => {
   ...
   function saveCoreLevelsToLocalStorage() {
     ...
     try {
       localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify({ coreLevels: coreLevelsById, ownedResources: playerOwnedResources }));
     } catch (error) {
   ...
   function loadCoreLevelsFromLocalStorage() {
     ...
     try {
       ...
       if (rawStoredValue) {
         ...
         applyImportedOwnedResources(parsedValue.ownedResources);
       }
     } catch (error) {
   ...
   // 套用一份「erda / fragments」的持有資源物件，缺漏或格式不對時保持 0，不會讓頁面壞掉
   function applyImportedOwnedResources(candidateOwnedResources) {
     if (!candidateOwnedResources || typeof candidateOwnedResources !== 'object') return;
     playerOwnedResources.erda = clampNonNegativeInteger(candidateOwnedResources.erda);
     playerOwnedResources.fragments = clampNonNegativeInteger(candidateOwnedResources.fragments);
   }
   ...
   function loadBackupSlotIntoCurrentProgress(slotId) {
     ...
     applyImportedOwnedResources(backupData.ownedResources);
     syncOwnedResourceInputsToState();
     saveCoreLevelsToLocalStorage();
     renderHexagonPlate();
     refreshAllProgressSections();
